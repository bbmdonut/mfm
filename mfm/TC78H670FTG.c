...
static bool pd_pinSetup();
static bool pd_stepSerialSingle(bool direction);
...
bool pd_controlModeSelect( void )
{
  ...
  case PRODRIVER_MODE_CLOCKIN:
    ...
    {
      digitalWrite(settings.mode0Pin, HIGH);
    }
    else{
      digitalWrite(settings.mode0Pin, LOW);
    }
    ...
    {
      digitalWrite(settings.mode1Pin, HIGH);
    }
    else{
      digitalWrite(settings.mode1Pin, LOW);
    }
...
// retuns errorStat()
// The step will take clockDelay milliseconds
...
// see datasheet pg 9
bool pd_changeStepResolution( uint8_t resolution)
{
  // If user asks to change resolution, but we are already there, then
  // just return pd_errorStat()
  if(settings.stepResolution == resolution) return pd_errorStat();
...
bool pd_sendSerialCommand( void )
{
  ...
  for(int i = 0 ; i < 32 ; i++)
  {
    digitalWrite(settings.mode2Pin, HIGH); // data HIGH
    delayMicroseconds(1);
    if(bitRead(command, i))
    {
      digitalWrite(settings.mode0Pin, HIGH); // data HIGH
    }
    else{
  ...
  digitalWrite(settings.mode1Pin, HIGH); // data HIGH
  delayMicroseconds(1);
...
bool pd_stepSerial(uint16_t steps, bool direction, uint8_t stepDelay)
{
  pd_enable();
  for(int i = 0; i < steps ; i++)
  {
    if(pd_stepSerialSingle(direction) == false) return false;
    delay(stepDelay);
  }
  return true;
}
...
bool pd_stepSerialSingle(bool direction)
{
