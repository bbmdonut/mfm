...
Modified to also support driving Autonics MD5-HD14 5 phase stepper driver
*/
...
#define PRODRIVER_MODE_AUTONICS 2

// default Arduino digital pin numbers
#define PRODRIVER_DEFAULT_PIN_STBY   (2*32+4)
#define PRODRIVER_DEFAULT_PIN_EN     (2*32+1)
#define PRODRIVER_DEFAULT_PIN_MODE_0 (2*32+23)
#define PRODRIVER_DEFAULT_PIN_MODE_1 (2*32+3)
#define PRODRIVER_DEFAULT_PIN_MODE_2 (2*32+22)
#define PRODRIVER_DEFAULT_PIN_MODE_3 (2*32+2)
#define PRODRIVER_DEFAULT_PIN_ERROR  (2*32+5)
...
#define PRODRIVER_STEP_RESOLUTION_FIXED_1_1 8
...
// This is talking to different controller
#define PRODRIVER_MODE_AUTONICS 2
...
typedef struct
{
  ...
  bool pd_step(uint32_t steps, bool direction, float clockDelay); // returns ERR stat
  bool pd_stepSerial(uint16_t steps, bool direction, uint8_t stepDelay); // only 1:1 stepping, (no microstep support)
  bool changeStepResolution(uint8_t resolution); // only works with "variable" step modes
  bool pd_controlModeSelect( void );
  bool pd_enable( void );
  bool pd_disable( void );
  bool pd_sendSerialCommand( void );
  bool pd_setTorque( uint8_t newTorque );
  bool pd_setCurrentLimit( uint16_t currentLimit );
